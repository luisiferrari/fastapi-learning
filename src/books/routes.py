...
@book_router.get("/", response_model=List[Book], dependencies=[Depends(access_token_bearer)])
...
async def get_book_by_id(book_id: uuid.UUID, session: AsyncSession = Depends(get_session), user_details = Depends(access_token_bearer)):
...
async def update_book(book_id: str, book_update: BookUpdateModel, session: AsyncSession = Depends(get_session), user_details = Depends(access_token_bearer)):
